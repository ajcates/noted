...
- Respond ONLY with the JSON object. Do not include any other text before or after the JSON.`;
  ...
  router.post('/process', async (ctx) => {
    const { promptId, fullContent, selection, history = [] } = ctx.request.body as any;
    ...
    switch (promptId) {
      case 'restructure':
        userPrompt = `Restructure the following note to improve logical flow and structural clarity. Group related ideas, use headings, and bulleted lists where appropriate. Focus on the selection if provided.\n\nSelection: ${selection || 'None'}\n\nFull Content:\n${fullContent}`;
        break;
      case 'clarify':
        userPrompt = `Clarify the following note by simplifying complex language and rephrasing jargon or convoluted sentences. Focus on the selection if provided.\n\nSelection: ${selection || 'None'}\n\nFull Content:\n${fullContent}`;
        break;
      case 'elaborate':
        userPrompt = `Elaborate on the following note by adding depth, explanations, and context. Build upon existing ideas with examples. Focus on the selection if provided.\n\nSelection: ${selection || 'None'}\n\nFull Content:\n${fullContent}`;
        break;
      case 'add_info':
        userPrompt = `Identify and fill in missing gaps in the following note. Inject relevant facts, definitions, or supplementary details. Focus on the selection if provided.\n\nSelection: ${selection || 'None'}\n\nFull Content:\n${fullContent}`;
        break;
      case 'add_perspective':
        userPrompt = `Introduce alternative viewpoints, counter-arguments, or different professional/cultural angles to the following note. Focus on the selection if provided.\n\nSelection: ${selection || 'None'}\n\nFull Content:\n${fullContent}`;
        break;
      case 'fact_check':
        userPrompt = `Fact check the following note for accuracy and logical soundness. Highlight potentially inaccurate claims and suggest corrections. Focus on the selection if provided.\n\nSelection: ${selection || 'None'}\n\nFull Content:\n${fullContent}`;
        break;
      case 'better_suggestion':
        userPrompt = `Generate 4 distinct stylistic variations (e.g., professional, casual, concise, persuasive) for the following content. Present them ONLY as options in the "questions" field of your JSON response, with the question being "Which style do you prefer?". Set "content" to null in your response.\n\nSelection: ${selection || 'None'}\n\nFull Content:\n${fullContent}`;
        break;
      case 'last_line':
        const lines = fullContent.trim().split('\n');
        const lastLine = lines[lines.length - 1];
        const precedingContent = lines.slice(0, -1).join('\n');
        userPrompt = `Execute the following command on the preceding content. Replace or append the result as appropriate.\n\nCommand: "${lastLine}"\n\nPreceding Content:\n${precedingContent}`;
        break;
      case 'chat':
        userPrompt = `User message: ${selection || ''}\n\nNote Context:\n${fullContent}`;
        break;
      default:
        userPrompt = `Process the following note based on user request. Focus on selection if provided.\n\nSelection: ${selection || 'None'}\n\nFull Content:\n${fullContent}`;
    }
